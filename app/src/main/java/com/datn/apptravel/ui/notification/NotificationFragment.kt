...
class NotificationFragment : BaseFragment<FragmentNotificationBinding, NotificationViewModel>() {
            ...
            viewModel.allNotifications.collect { allNotifications ->
                try {
                    ...
                    try {
                        ...
                        android.util.Log.e("NotificationFragment", "Error updating badge: ${e.message}", e)
                    }
                } catch (e: Exception) {
                    android.util.Log.e("NotificationFragment", "❌ Error in observeNotifications: ${e.message}", e)
                    Toast.makeText(requireContext(), "Lỗi hiển thị thông báo", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun setupDotsIndicator(totalPages: Int) {
        try {
            ...
            for (i in 0 until totalPages) {
                val dot = ImageView(requireContext()).apply {
                    try {
                        ...
                        android.util.Log.e("NotificationFragment", "Missing ic_circle drawable", e)
                        // Use a simple colored view as fallback
            ...
            android.util.Log.e("NotificationFragment", "❌ Error setting up dots: ${e.message}", e)
        }
    }
